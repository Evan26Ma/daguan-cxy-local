(() => {
  ...
  async function checkRuntimeVersion() {
    try {
      ...
      if (!runtime.appVersion) return;
      let seen = "";
      try { seen = localStorage.getItem(RUNTIME_SEEN_KEY) || ""; } catch { /* storage unavailable */ }
      // 版本与服务端不一致且用户还没确认过这一版时才提醒，避免横幅常驻。
      if (runtime.appVersion !== APP_VERSION && runtime.appVersion !== seen) {
  ...
  function bindUI() {
    ...
    $("#btn-update-refresh")?.addEventListener("click", async () => {
      ...
      try {
        ...
        if (response.ok) {
          const runtime = await response.json();
          if (runtime.appVersion) localStorage.setItem(RUNTIME_SEEN_KEY, runtime.appVersion);
        }
      } catch { /* ignore */ }
